...
// Above is for AWS, no DB:s for Azure yet. This function will also fail.
...
const sendMessageToClient = (payload, id) =>
  new Promise((resolve, reject) => {
    const apigatewaymanagementapi = new AWS.ApiGatewayManagementApi({
      apiVersion: '2029',
      endpoint: 'https://3owhikvdr6.execute-api.eu-north-1.amazonaws.com/prod'
    })
...
async function main(context) {
  let body = JSON.parse(context.req.body)
  let organization = body.organization.login
  let gitEvent = context.req.headers['X-GitHub-Event']
      ...
      if (settings[gitEvent]) {
        ...
        if (userConnected.Count && userConnected.Count !== 0) {
          ...
          await webPush.sendNotification(
            JSON.parse(serviceWorker.Item.worker),
            JSON.stringify({
